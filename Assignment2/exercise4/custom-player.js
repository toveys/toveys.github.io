...
//This is where I call the progress bar class and assign it a variable
let progressBar = document.getElementsByClassName("progress-bar")[0];
//I had a problem with setting the progressbar max value here since it seemed to get set before the media loaded
// so I declare a mediaload bool for later.
var mediaload=false;
...
function setTime(){
  ...
  //I replace the grey progress bar with a range and set the range's current value to update 
  //with the media current time value every update log. 
  progressBar.value = media.currentTime;
  //I use the mediaload bool here for a one time if statement. Once the media starts playing, or you click on the progress bar, the time will
  //update which triggers the main function. This is handy since right at the beginning this main function will be triggered
  //and I can activate this if statement and set the progressbar max value with certainty that the media has been loaded.
  if(mediaload==false){
    //I gave the progressBar maximum value a default value in the html, but here I make it scaleable
    //Any video passed through, I can grab the media max duration value and assign it as the progress bar maximum value
    progressBar.max = media.duration;
    mediaload=true;
  }
}

//Here I give the progressbar range functionality, when it is changed it sets the media's current time
//to the value that you change the range to
progressBar.addEventListener("input", (e) =>{changeTime(e.target.value)});
